...
const candidateId = '611ef6c0-8828-4401-8546-bf5536423019';
const profileHash = '2AD5D028B86C23EFEE6827224F5FF6B6D011F265151DFFBEC2C238473DCF70CE';
const tuple = { winget_id: 'WINBIGFOX.TimeScribe', version: '1.16.0', architecture: 'x64',
  installer_sha256: '7F8A4729661150B7A1A9E4E3F3FE347BFA17507AED3096806DABD0A1F34A706B' };
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35078027976' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35078027976 returned 0/0/60001/0 under LocalSystem. Exact captured NSIS registration 932b644f-cf07-5d84-aef8-0b37bf9d7ce1 pointed to a missing systemprofile LocalAppData/Programs/timescribe/Uninstall timescribe.exe. Managed removal could not run and detection remained positive. Vendor NativePHP 2.3.1 uses default one-click NSIS settings. No verified LocalSystem replacement removal route is established. Contain this exact payload until a reviewed shared lifecycle repair and controlled retest succeed. VirusTotal was clean (0/0).',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-timescribe-shared-quarantine',
  });
