...
const tuple = { winget_id: 'Opera.Opera', version: '136.0.6008.52', architecture: 'x64',
  installer_sha256: 'E628250756E8B7AD9CDE787DFAE806AA2929CA66B77F23D72020BEEDCFB8D1F9' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact Opera pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '36010747225' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 36010747225 returned 0/0/60001/0 under LocalSystem. The reviewed exact ProgramFiles/Opera/opera.exe command --uninstall --runimmediately --deleteuserprofile=0 launched and its parent exited, but exact registration Opera 136.0.6008.52 remained beyond the five-minute deadline. Removal detection remained positive. Hold only this exact payload until a reviewed shared repair and strict retest. VirusTotal was clean 0/0.',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-opera-136-shared-quarantine',
  });
