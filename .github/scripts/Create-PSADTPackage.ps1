...
if ($useManagedDirectoryLifecycle) {
    ...
    if ($originalInstallerType -eq 'burn') {
        ...
        $lines += @(
            ...
            '        $isInstall4jUninstall = ($registeredUninstallLeaf -ieq ''uninstaller.exe'' -and $registeredUninstallParentLeaf -ieq ''.install4j'')'
            '        if ($isInstall4jUninstall) {'
            '            # install4j uses -q for unattended installers and uninstallers. The .install4j parent'
            '            # plus the canonical uninstaller leaf is a framework signature, so this does not'
            '            # broaden silent arguments to arbitrary vendor executables.'
