...
function profileMatches(candidate) {
  const canonical = candidate.test_config?.packageProfileCanonicalJson;
  if (typeof canonical !== 'string' || createHash('sha256').update(canonical).digest('hex').toUpperCase() !== candidate.package_profile_sha256) return false;
  const p = JSON.parse(canonical);
  return candidate.test_level === 'psadt-package' && candidate.test_config?.mode === 'psadt-package' &&
    p.testLevel === 'psadt-package' && p.toolchain?.packagerCommit === pin &&
    p.app?.wingetId === 'Kiwix.Wikivoyage.Electron' && p.app?.version === '3.8.2-E' && p.app?.architecture === 'x86' &&
    p.installer?.sha256 === '353BFC413A3787D7CEA738AB8BAAAB7E33AF42883963D2C29DDEDA438BFFF0C0' &&
    p.installer?.installScope === 'machine' &&
    p.installer?.uninstallCommand === 'REGISTRY_UNINSTALL_KEY:149170a6-d630-5e6f-a054-8c34dd8a32a2:Wikivoyage by Kiwix';
}
function freshControl(control) {
  const seen = Date.parse(control.scheduler_seen_at);
  return control.required_packager_commit === pin && control.scheduler_packager_commit === pin &&
    Number.isFinite(seen) && seen <= Date.now() && Date.now() - seen <= 300000;
}
...
if (action==='pause-retry') {
  const active=await request('qa_candidates',{status:'in.(dispatched,running)',select:'id,winget_id,version,architecture,installer_sha256,package_profile_sha256,test_level,test_config'});
  if(active.length!==1 || active[0].winget_id!=='Kiwix.Wikivoyage.Electron' || active[0].version!=='3.8.2-E' ||
    active[0].architecture !== 'x86' || active[0].installer_sha256 !== '353BFC413A3787D7CEA738AB8BAAAB7E33AF42883963D2C29DDEDA438BFFF0C0' ||
    !profileMatches(active[0])) throw new Error('Exact active retry required');
...
if (action==='set-pin') {
  ...
  if(!freshControl(control)) throw new Error('Fresh aligned scheduler pin required');
  const candidates=await request('qa_candidates',{winget_id:'eq.Kiwix.Wikivoyage.Electron',version:'eq.3.8.2-E',architecture:'eq.x86',status:'eq.queued',installer_sha256:'eq.353BFC413A3787D7CEA738AB8BAAAB7E33AF42883963D2C29DDEDA438BFFF0C0',select:'id,priority,package_profile_sha256,test_level,test_config'});
  const matches=candidates.filter(profileMatches);
  ...
  if(action==='prioritize-retry') {
    ...
    if(!freshControl(await pausedIdle())) throw new Error('Fresh aligned scheduler pin required');
    const updated=await request('qa_pipeline_control',{id:'eq.global',paused:'eq.true',reason:`eq.${repairReason}`,required_packager_commit:`eq.${pin}`,scheduler_packager_commit:`eq.${pin}`,and:`(scheduler_seen_at.gte.${new Date(Date.now()-300000).toISOString()},scheduler_seen_at.lte.${new Date().toISOString()})`},
  ...
  const candidates=await request('qa_candidates',{winget_id:'eq.Kiwix.Wikivoyage.Electron',version:'eq.3.8.2-E',architecture:'eq.x86',installer_sha256:`eq.${hash}`,status:'eq.passed',test_level:'eq.psadt-package',select:'id,package_profile_sha256,test_level,test_config'});
  const matching=candidates.filter(profileMatches);
