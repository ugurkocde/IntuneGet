...
const candidateId = '96aa05ec-f02d-453b-8f64-3868c997ff83';
const profileHash = '409398279CAA0EEF7F7D03E0022C424302F2FFC61B2B4AC1F04D3B998FCAD87A';
const tuple = { winget_id: 'Thunder.Thunder', version: '25.1.13.1637', architecture: 'x64',
  installer_sha256: 'B2C7A5269B267E7390BED95975FF9BA56088B26A945B6A2BA4B35B3B15FE8EC6' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact Thunder pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35223559453' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
  ...
  !c.live_log?.lines?.some(line => typeof line === 'string' && line.includes('The vendor uninstall command did not remove registration [thunder_is1] before the completion deadline.')) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35223559453 returned 0/0/60001/0 under LocalSystem. The exact thunder_is1 registration remained after the registered Program Files/Thunder Network/Thunder/Program/ThunderUninstall.exe ran and the 310-second completion deadline expired. Independent removal detection still found the app. This exact payload requires a reviewed lifecycle repair and controlled retest before release. VirusTotal had no verdict (not_found); reputation is unverified and cannot count as a strict pass.',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-thunder-shared-quarantine',
  });
