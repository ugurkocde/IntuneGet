...
const candidateId = '11be0474-0a96-4ae9-ac16-ee53d6ec94aa';
const profileHash = 'A42BFD5603960ECF8D3481046EC205A7B90A2438EFFB0674C3CD1AD259BDA17A';
const tuple = { winget_id: 'T3Tools.T3Code', version: '0.0.42', architecture: 'x64',
  installer_sha256: '9BD4A00AE9B4880F85E81376844E4FC1DBC9F719120958D7445B4C2B281E267F' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact T3Code pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35214994473' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
  ...
  !c.live_log?.lines?.some(line => typeof line === 'string' && line.includes('The registered vendor uninstaller was not found:')) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35214994473 returned 0/0/60001/0 under LocalSystem. Exact registration e9197887-efb3-55e0-985e-d6d3b5dd594a pointed to a missing systemprofile LocalAppData/Programs/t3code/Uninstall T3 Code (Alpha).exe. Managed removal could not run. This exact payload requires a reviewed lifecycle repair and controlled retest before release. VirusTotal had no verdict (not_found); reputation is unverified and cannot count as a strict pass.',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-t3code-shared-quarantine',
  });
