...
export default function SignInClient({ callbackUrl }: { callbackUrl: string }) {
  ...
  const [showVerificationScene, setShowVerificationScene] = useState(false);
  const verificationActive = isAuthenticated || isVerifyingConsent;
  useEffect(() => {
    if (!verificationActive || shouldReduceMotion) {
      setShowVerificationScene(false);
      return;
    }
    ...
    if (device.connection?.saveData ||
        ...
        ['slow-2g', '2g'].includes(device.connection?.effectiveType || '')) return;
    const timer = window.setTimeout(() => setShowVerificationScene(true), 2500);
    return () => window.clearTimeout(timer);
  }, [verificationActive, shouldReduceMotion]);
  ...
  if (isAuthenticated || isVerifyingConsent) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-bg-deepest">
        {!showVerificationScene || shouldReduceMotion ? (
