...
const source = readFileSync(new URL('./qa-timescribe-1171-quarantine.mjs', import.meta.url), 'utf8')
...
const sha = '6DBADEF47A4ED4ADA5BEFA798462A6571D2B5525AC9FA1E5E96F202469C645E1';
...
async function execute(action: string, options: {
  ...
  const canonical = JSON.stringify({ app: { wingetId: 'WINBIGFOX.TimeScribe', version: '1.17.1', architecture: 'x64' },
  ...
  const fetch = vi.fn(async (url: string, init: { method: string; body?: string }) => {
    ...
    const data = table === 'qa_pipeline_control' ? [control]
      ...
      : [{ id: candidateId, github_run_id: '35890849220', package_profile_sha256: profileHash,
...
describe('TimeScribe guarded exact-payload containment', () => {
  it('writes only the exact block and safely undispatched matching rows', async () => {
    ...
    expect(writes[0].body).toMatchObject({ winget_id: 'WINBIGFOX.TimeScribe', version: '1.17.1',
    ...
    expect(Object.fromEntries(writes[1].params)).toMatchObject({ status: 'eq.queued', dispatched_at: 'is.null',
      github_run_id: 'is.null', installer_sha256: `eq.${sha}`, version: 'eq.1.17.1' });
  });
