...
const candidateId = 'cd07305e-2c4e-4f5b-90ca-4199467dbec9';
const profileHash = '3BC080CDEE5AA04095DD58D31DE0184A53F8EA35568AC47A2590A8B5F553D056';
const tuple = { winget_id: 'SKCommunications.NateOn', version: '7.0.41.0', architecture: 'x86',
  installer_sha256: '1DCA7E3230CDB6BEC7374DEE2D226D62C73919B6119D870DD6BC29D19915AE2F' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact NateOn pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35166642072' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35166642072 returned 0/0/60001/0 under LocalSystem. Exact captured registration {EA77EC9A-C82F-4F80-8B7D-D32C09A9C25F} remained after the registered NATEON/Bin/uninstall.exe /S command exited and the 310-second completion deadline elapsed. WinGet declares Nullsoft; the documented NSIS silent switch was already used. No verified alternative managed removal route is established. Contain this exact payload until reviewed shared removal and a controlled strict retest succeed. VirusTotal was not_found with null counts, not a clean verdict.',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-nateon-shared-quarantine',
  });
