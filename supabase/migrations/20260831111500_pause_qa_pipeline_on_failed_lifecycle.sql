-- A lifecycle failure is a material production QA event. Pause the global
-- dispatcher in the same transaction that records the terminal candidate so
-- no later candidate can start before an operator or automation has diagnosed
-- and safely dispositioned the failure.
...
begin
  ...
  if normalized_outcome = 'failed' then
    update public.qa_pipeline_control
    set paused = true,
        reason = left(
          format(
            'Automated QA failure: %s %s (%s), candidate %s. %s',
            candidate.winget_id,
            candidate.version,
            candidate.architecture,
            candidate.id,
            coalesce(nullif(p_summary, ''), 'No summary provided.')
          ),
          1000
        ),
        updated_at = now(),
        updated_by = 'qa-result-fail-close'
    where id = 'global'
      and paused = false;

    if not exists (
      select 1
      from public.qa_pipeline_control
      where id = 'global'
        and paused = true
    ) then
      raise exception 'Global QA pipeline control is missing or could not be paused';
    end if;
  end if;
