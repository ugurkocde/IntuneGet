...
const candidateId = 'c8cb27b7-6191-4228-9a3a-a03f12d486ff';
const profileHash = 'A2618A5F30CA6CCAC2EA53002F5EF40BE7ED43EAD4D4D762D75A126B050EBBD2';
const tuple = { winget_id: 'luqiangbo.DockMapper', version: '1.1.5', architecture: 'x64',
  installer_sha256: '2C17B07EA68C59D38FCE1DACCD88F294FF6E018DC2A87355E95771CC0F141D50' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact DockMapper pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35460300940' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35460300940 returned 0/0/60001/0 under LocalSystem. Exact captured NSIS registration DockMapper pointed to a missing systemprofile LocalAppData/DockMapper/uninstall.exe. Managed removal could not run and detection remained positive. Vendor v1.1.5 Tauri configuration specifies currentUser NSIS installation. No verified LocalSystem replacement removal route is established. Contain this exact payload until a reviewed shared lifecycle repair and controlled retest succeed. VirusTotal was clean (0/0).',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-dockmapper-shared-quarantine',
  });
