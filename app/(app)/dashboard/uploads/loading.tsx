      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex items-center gap-2">
          {['All', 'Pending', 'Processing', 'Completed', 'Failed'].map((_, i) => (
            <div key={i} className="h-8 w-20 bg-overlay/5 rounded-lg animate-pulse" />
          ))}
        </div>
          ...
          <div key={i} className="glass-light rounded-xl p-6 border border-overlay/5">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="h-5 w-48 bg-overlay/5 rounded animate-pulse mb-2" />
                  <div className="h-4 w-32 bg-overlay/5 rounded animate-pulse" />
                </div>
              </div>
              <div className="h-6 w-24 bg-overlay/5 rounded-full animate-pulse" />
            </div>
            <div className="h-2 w-full bg-overlay/5 rounded-full animate-pulse" />
          </div>
        ))}
      </div>
    </div>
