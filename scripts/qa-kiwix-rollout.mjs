...
if (action==='pause-retry') {
  ...
  const updated=await request('qa_pipeline_control',{id:'eq.global',paused:'eq.false',required_packager_commit:`eq.${pin}`},
    {paused:true,reason:'Exact repaired application validation in progress.',updated_at:new Date().toISOString(),updated_by:'qa-kiwix-retry-audit'});
...
if (action==='set-pin') {
  ...
  const updated=await request('qa_pipeline_control',{id:'eq.global',paused:'eq.true',required_packager_commit:`eq.${oldPin}`},
    {required_packager_commit:pin,updated_at:new Date().toISOString(),updated_by:'qa-kiwix-reviewed-rollout'});
