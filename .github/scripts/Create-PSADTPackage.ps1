...
if (-not [string]::IsNullOrWhiteSpace($customUninstallCommand)) {
    ...
    if ($originalInstallerType -eq 'burn') {
        ...
        $lines += @(
            ...
            "            `$safeManifestUninstallArguments = @('$silentSwitchesEscaped' -split '\s+' | Where-Object { `$_ -match '^(?i:/q[nbrfu]?|/quiet|/silent|/verysilent|/norestart|/s|--quiet|--silent|-s)$' })"
            ...
            '        $vivaldiUninstaller = [string]$registeredApplication.UninstallStringFilePath'
            '        if (-not (Test-Path -LiteralPath $vivaldiUninstaller -PathType Leaf)) {'
            '            throw "The registered Vivaldi uninstaller was not found: $vivaldiUninstaller"'
            '        }'
            ...
            '        Start-ADTProcess -FilePath $vivaldiUninstaller -ArgumentList $vivaldiUninstallArguments -WorkingDirectory (Split-Path -Parent $vivaldiUninstaller) -CreateNoWindow -SuccessExitCodes @(0, 1605, 1614) -RebootExitCodes @(1641, 3010)'
            '    } elseif (-not $hasQuietUninstall) {'
            '        # Some vendor uninstallers remove their exact ARP entry successfully but keep the'
            '        # bootstrapper process open. Launch the captured command asynchronously and use the'
            '        # captured registry identity as the authoritative completion signal.'
            '        $registeredUninstallFile = [string]$registeredApplication.UninstallStringFilePath'
            ...
            '        }'
            '        [string[]]$registeredUninstallArguments = @($registeredApplication.UninstallStringArgumentList | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })'
            ...
            '                if ($registeredUninstallArguments[$argumentIndex] -match ''(?i)^/i(?=\{|$)'') {'
            '                    $registeredUninstallArguments[$argumentIndex] = $registeredUninstallArguments[$argumentIndex] -replace ''(?i)^/i'', ''/x'''
            '                }'
            '            }'
            '            foreach ($argument in @(''/qn'', ''/norestart'')) {'
            '                if ($registeredUninstallArguments -notcontains $argument) { $registeredUninstallArguments += $argument }'
            '            }'
            '        }'
            ...
            '        $uninstallHandle = Start-ADTProcess -FilePath $registeredUninstallFile -ArgumentList $registeredUninstallArguments -WorkingDirectory $registeredUninstallWorkingDirectory -WindowStyle Hidden -NoWait -PassThru'
            '        $uninstallDeadline = [DateTime]::UtcNow.AddMinutes(5)'
            '        $uninstallProcessExitedAt = $null'
            ...
            '            if ($uninstallProcessExited) {'
            '                if (-not $uninstallProcessExitedAt) { $uninstallProcessExitedAt = [DateTime]::UtcNow }'
            '                if (([DateTime]::UtcNow - $uninstallProcessExitedAt).TotalSeconds -ge 15) { break }'
            '            } else {'
            '                $uninstallProcessExitedAt = $null'
            '            }'
