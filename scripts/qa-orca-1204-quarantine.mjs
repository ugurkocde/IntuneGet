...
const candidateId = 'bed73447-69a1-43a2-be37-4e403210c84d';
const profileHash = '46BA4C908D8E834FA6BF9458990745B466C179ECD9F2A73237CEA41F6CECA00C';
const tuple = { winget_id: 'StablyAI.Orca', version: '1.4.204', architecture: 'x64',
  installer_sha256: '87B877EC7472F664E5264DC5367A5F18F4484AEA67A08B4ECA76F9A65729206C' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact Orca pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35203768255' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35203768255 returned 0/0/60001/0 under LocalSystem. Exact registration 2b325ec9-0ed1-575f-ad70-e08307aee879 pointed to a missing systemprofile LocalAppData/Programs/orca/Uninstall Orca.exe. Managed removal could not run. This exact payload requires a reviewed lifecycle repair and controlled retest before release. VirusTotal was clean (0/0).',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-orca-shared-quarantine',
  });
