...
const candidateId = '23a97646-b8b4-4725-939f-a7e288bdbafa';
const profileHash = 'C8E2A3F03FDF912F478E3E7102A389D8529260F46B0C657FA765A699E793659B';
const tuple = { winget_id: 'SJMC.SJMCL', version: '1.3.1', architecture: 'x64',
  installer_sha256: 'D736C896A039A9AFB8B7D4339A79293FAAAC6EF3F164DAF2DD44F8702997178A' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact SJMCL pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35250591040' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
  ...
  profile.app?.wingetId !== tuple.winget_id || profile.app?.version !== tuple.version || profile.app?.architecture !== tuple.architecture ||
  profile.installer?.uninstallCommand !== 'REGISTRY_UNINSTALL:SJMCL' ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35250591040 returned 0/0/60001/0 under LocalSystem. Exact registration SJMCL pointed to a missing systemprofile LocalAppData/SJMCL/uninstall.exe. Managed removal could not run. This exact payload requires a reviewed lifecycle repair and controlled retest before release. VirusTotal had no verdict (not_found); reputation is unverified and cannot count as a strict pass.',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-sjmcl-shared-quarantine',
  });
