...
const candidateId = 'd49f33d4-3327-4f96-8304-efd18545d534';
const profileHash = '18F9281E28E649B7F9420D3FFB8339DE688667B00024BFE124C6561B6ABFD52A';
const tuple = { winget_id: 'XplicitTrust.Agent', version: '1.065', architecture: 'x64',
  installer_sha256: '9015EEE906A0B84F2B5B0471E6F7C88C5BCF50DE6B6F32C5EB252D385D7FDBD2' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact XplicitTrust pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35059738293' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35059738293 returned 0/0/60001/0 under LocalSystem. Installation captured the manifest MSI ProductCode {76CCDAB5-94FA-4CE5-9B0D-6F8304D801A3}, but uninstall found zero entries for that exact identity and safely refused broad removal. The marker still detected the application. Cause of the registration disappearance is unproven. Contain this exact payload until a reviewed managed lifecycle repair and controlled retest succeed. VirusTotal was clean (0/0).',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-xplicittrust-shared-quarantine',
  });
