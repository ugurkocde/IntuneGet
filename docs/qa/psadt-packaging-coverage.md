...
| 2026-08-19 | darktable 5.6.0 used the correct vendor NSIS `/S` switch and extracted thousands of files, but its installer stayed quiet beyond the generic four-minute inactivity window and was interrupted before final ARP registration. | Application-specific long-running install lifecycle; the initial `60001` uninstall result was secondary because the interrupted installer never registered | Failed in [run 32239291005](https://github.com/ugurkocde/IntuneGet-Workflows/actions/runs/32239291005). The shared customer and QA packager now uses a ten-minute bounded asynchronous wait with PSADT progress heartbeats for darktable while preserving its official `/S` install and exact ARP uninstall contract. Exact-profile QA rerun required. |
