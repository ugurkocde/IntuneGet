...
const candidateId = '3849c8a1-5bd3-4c68-b44d-14c5aefe018b';
const profileHash = 'B31D704CF0CA41B4D71AA14133DD9F1E3A100C13E93B354E5CB2FAE8AB3A33FF';
const tuple = { winget_id: 'Ubiquiti.UniFiNetworkServer', version: '10.6.106', architecture: 'x64',
  installer_sha256: '984FEFAA18AA38D90928F9159D2F2C8286202F19B0E038E3C2A8F7192DFC1C91' };
...
async function guard() {
  ...
  if (controls.length !== 1 || !c.paused || active.length || c.required_packager_commit !== pin || c.scheduler_packager_commit !== pin ||
    !c.reason?.includes(`candidate ${candidateId}.`)) throw new Error('Exact UniFi Network pause, zero active lifecycles and aligned pin required');
  return c;
}
...
if (candidates.length !== 1 || results.length !== 1 || c.github_run_id !== '35097308073' || (r.github_run_id != null && String(r.github_run_id) !== c.github_run_id) ||
  ...
  profile.testLevel !== 'psadt-package' || profile.app?.wingetId !== tuple.winget_id ||
...
if (action === 'block') {
  ...
  await request('qa_package_blocks', { on_conflict: 'winget_id,version,architecture,installer_sha256' }, 'POST', {
    ...
    detail: 'Isolated PSADT run 35097308073 returned 0/0/60001/0 under LocalSystem. Exact captured NSIS registration Ubiquiti UniFi pointed to a missing systemprofile/Ubiquiti UniFi/Uninstall.exe. Managed removal could not run and detection remained positive. Official WinGet declares machine scope and a Profile-based installation location. Vendor documentation directs removal through that same registered entry and interactive prompts. No verified replacement removal route is established. Contain this exact payload until a reviewed shared lifecycle repair and controlled retest succeed. VirusTotal was clean (0/0).',
...
if (action === 'resume') {
  ...
  const updated = await request('qa_pipeline_control', { id: 'eq.global', paused: 'eq.true', reason: `eq.${fresh.reason}`,
    ...
    paused: false, reason: null, updated_at: new Date().toISOString(), updated_by: 'qa-unifi-network-shared-quarantine',
  });
